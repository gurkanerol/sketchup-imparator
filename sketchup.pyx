...
cdef class Entities:
    ...
    def get_textured_ngon_lists(self, default_material):
        verts = []
        faces_data = [] # List of (mat_index, [loops]) where each loop is a triangle in this pass
        mats = {}
        seen = {}
        uv_data = [] # List of loops of UV coordinates
        
        cdef Face face_item
        cdef Material mat_item
        cdef Texture tex_item
        cdef double s_scale, t_scale
        ...
        cdef size_t* tri_indices
        cdef SUPoint3D* mesh_verts
        cdef SUPoint3D* mesh_stq
        cdef double q
        cdef size_t t, k
        ...
        for f in self.faces:
            ...
            if face_item.material:
                mat_item = face_item.material
                mat_name = mat_item.name
                if mat_item.texture:
                    tex_item = mat_item.texture
                    # Accessing property dimensions from Python context in Cython
                    dim = tex_item.dimensions
                    s_scale = dim[2]
                    t_scale = dim[3]
            # Support for backface materials if front is default (Option 3)
            # This is crucial for models where only the 'inside' of a wall is painted
            if not face_item.material:
                # We access face.back_material (if available in the wrapper)
                try:
                    # Looking at the original wrapper, we might need to get back material
                    back_mat = face_item.back_material
                    if back_mat:
                        mat_item = back_mat
                        mat_name = mat_item.name
                        if mat_item.texture:
                            ...
                            s_scale = dim[2]
                            t_scale = dim[3]
                            # Use back STQ coords if using back material
                            # NOTE: We'll implement this if possible, or stick to front for now
                except:
                    pass
            ...
            # Use MeshHelper for robust triangulation (this respects holes perfectly)
            mesh_ref.ptr = <void*> 0
            ...
            # Bulletproof Memory Management: initialize to NULL
            tri_indices = NULL
            mesh_verts = NULL
            mesh_stq = NULL
            ...
            try:
                ...
                mesh_stq = <SUPoint3D*> malloc(v_count * sizeof(SUPoint3D))
                ...
                # If we are using back material, we should ideally get back STQ coords
                # For simplicity and robust N-gons, we use front coords but normalized
                check_result(SUMeshHelperGetFrontSTQCoords(mesh_ref, v_count, mesh_stq, &got_stq))
                ...
                for t in range(tri_count):
                    ...
                    for k in range(3):
                        ...
                        q = mesh_stq[v_idx].z
                        if abs(q) < 1e-6: q = 1.0
                        # Fix: Round UVs to 6 decimals for perfect cross-computer consistency
                        u = round(mesh_stq[v_idx].x / q * s_scale, 6)
                        v = round(mesh_stq[v_idx].y / q * t_scale, 6)
                        tri_uvs_list.append((u, v))
                    
                    face_tri_loops.append(tri_indices_list)
                    face_tri_uvs.append(tri_uvs_list)
                    
                faces_data.append((mat_number, face_tri_loops))
                uv_data.append(face_tri_uvs)
            finally:
                # Always free memory to prevent leak in case of error
                if tri_indices: free(tri_indices)
                if mesh_verts: free(mesh_verts)
                if mesh_stq: free(mesh_stq)
                SUMeshHelperRelease(&mesh_ref)
            
        return verts, faces_data, uv_data, mats
